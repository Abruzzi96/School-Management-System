...
public class Main {
	...
	private static HashMap<String, Manager> createManagerMap(String managersFile) {
		...
		File myManagerFile = new File(managersFile);

		try {
			Scanner scanner = new Scanner(myManagerFile);
			scanner.nextLine();
			while(scanner.hasNextLine()) {
				String[] read = scanner.nextLine().split(";");
	...
	public static HashMap<String, Student> createStudentMap(String studentsFile){
		...
		try {
			...
			while(scanner.hasNextLine()) {
				...
				HashMap<String, Student> newAdvisedStudentList = instructorList.get(advisorID).getAdvisedStudents();
				newAdvisedStudentList.put(ID, student);
				instructorList.get(advisorID).setAdvisedStudents(newAdvisedStudentList);
				// student yarattık
				for(int i = 0; i < curriculumList.get(department).size(); i++) {
					if(curriculumList.get(department).get(i).getSemester() < classlevel*2) {
						ArrayList<CourseNode> studentCompletedCourses = student.getTranscript().getCompletedCourses();
						studentCompletedCourses.add(new CourseNode(curriculumList.get(department).get(i)));
						
						Transcript studentTranscript = student.getTranscript();
						studentTranscript.setCompletedCourses(studentCompletedCourses);
					...
					if(curriculumList.get(department).get(i).getSemester() == classlevel*2) {
						...
						studentTakenCourses.add(new CourseNode(curriculumList.get(department).get(i),true));
